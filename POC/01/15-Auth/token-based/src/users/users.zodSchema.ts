...
const userSchema = z.object({
  ...
  email: z
    ...
    .email(), // set unique in create user schema
  password: z
    .string({
  ...
  role: z.nativeEnum(Roles).readonly().default(Roles.Normal)
});
...
const createUserSchema = z
  .object({
    ...
    email: z
      ...
      .email()
      .refine((data) => isUnique(data), {
        message: 'Email already in use!',
      }), // set unique in create user schema
    password: z
      .string({
        required_error: 'Password is required',
        invalid_type_error: 'Password must be a string',
...
const isUnique = async (data: string) => {
...
export { UserType, userSchema, createUserSchema, loginUserSchema };
