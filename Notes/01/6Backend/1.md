...
Stateless
Cacheable
Client-Server -> Server and client are independent on each other.
Layered System
Code on Demand 
...
// Middleware -- to read body content using POST
...
GET /user/<id> - Get user with the given id
...
PUT - to use when we are uploading something or putting a value which is missing, uploading resume
